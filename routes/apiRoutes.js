...
module.exports = function (app) {
  ...
  app.get('/api/overlap', function (req, res) {
    ...
    sequelize
      .query(
        'SELECT t2.*, t1.id overlapWithEventID, t1.UserID chosenUser, t3.username eventUserName FROM events t1 JOIN events t2 JOIN users t3 WHERE t1.UserID = ' +
          userNum +
          ' AND t3.id = t2.UserID AND t2.UserID != ' +
          userNum +
          ' AND date(t1.end) >= CURDATE() AND TIMESTAMPDIFF(MINUTE, GREATEST(t1.start, t2.start), LEAST(t1.`end`, t2.`end`)) >= 30;',
