...
class Messenger extends Component {
  ...
  connectToSocket = () => {
    socket.on('output', (data) => {
      ...
      this.setState({
        ...
        showMessages: allMessages.filter(
          (message) => message.recipientId == this.state.sendTo.id || message.senderId == this.state.sendTo.id
        ),
  ...
  handleInputChange = (event, newValue) => {
    if (event.type === 'click') {
      ...
      this.setState({
        sendTo: { id: parseInt(recipientId), username: recipientUsername, active: false },
        room: room,
        showMessages: this.state.allMessages.filter(
          (message) => message.recipientId == recipientId || message.senderId == recipientId
        )
      })
  ...
  handleNewChange = (event, newValue) => {
    if (newValue.id !== this.state.sendTo.id) {
      ...
      this.setState({
        ...
        room: room,
        showMessages: this.state.allMessages.filter(
          (message) => message.recipientId == newValue.id || message.senderId == newValue.id
        )
      })
                ...
                <List>
                  ...
                  {this.state.conversations.map((conversation) => (
                    <Paper>
                      ...
                      {conversation.senderId !== this.state.user.userid && conversation.read == false ? (
                        <ListItem onClick={this.handleInputChange} className={classes.unreadMessage} button>
                          <ListItemText
                            primary={
                              conversation.User.username === this.state.user.username
                                ? conversation.recipient.username
                                : conversation.User.username
                            }
                            secondary={conversation.message}
                            data-id={
                              conversation.senderId === this.state.user.userid
                        ...
                        <ListItem onClick={this.handleInputChange} className={classes.readMessage} button>
                          <ListItemText
                            primary={
                              conversation.User.username === this.state.user.username
                                ? conversation.recipient.username
                    ...
                    {this.state.showMessages.map((message, idx) => (
                      <Paper key={idx}>
                        {message.senderId == this.state.user.userid ? (
                          ...
                          <ListItem className={classes.listItemThem} button>
                            <ListItemText
